...
* @Last Modified time: 2019-09-07 10:59:51
*/
// 节点树
		...
		*/
		...
		renderAuth: function(tree, dept, opt){
			...
			layui.each(tree, function(index, item){
				...
				str += '<div class="auth-skin">';
				str += '<div class="auth-status" style="display: flex;flex-direction: row;align-items: flex-end;"> ';
				if(hasChild){

				    str += '<i class="layui-icon auth-icon '+(openstatus?'active':'')+'" style="cursor:pointer;">'+(openstatus?obj.openIconContent:obj.closeIconContent)+'</i>';
				} else {

				    str += '<i class="layui-icon auth-leaf" style="opacity:0;color: transparent;">&#xe626;</i>';
				}

				if(dept > 0){

				    str += '<span>'+opt.prefixChildStr+' </span>';
				} else {

				    str += '';
				}
				str += '<input class="authtree-checkitem"';
				str += ' type="'+opt.checkType+'"';
				str += ' name="'+inputname+'"';
				str += ' title="'+item[nameKey]+'"';
				str += ' value="'+item[valueKey]+'"';
				str += ' lay-skin="primary"';
				str += ' lay-filter="'+layfilter+'"'
				str += (isChecked?' checked="checked"':'')
				str += (isDisabled?' disabled':'')
				str += '>';
				str += '</div>';
				str += '<div class="auth-child" style="'+(openstatus ?'':'display:none;')+'padding-left:40px;"> '+append+'</div></div>';
			});
			str += '</div>';
			return str;
		},
